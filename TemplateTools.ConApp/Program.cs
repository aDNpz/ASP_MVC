...
namespace TemplateTools.ConApp
{
    partial class Program
    {
            ...
            while (input.Equals("x") == false)
            {
                ...
                if (Int32.TryParse(input, out var select))
                {
                    if (select == 1)
                    {
                        ...
                        var qtSolutions = Program.GetQTSchoolSolutions(Program.UserPath).Union(new[] { solutionPath }).ToArray();
        ...
        internal static string[] GetQTSchoolSolutions(string startPath)
        {
            var result = new List<string>();
            var qtPaths = GetQTSchoolProjects(startPath);
        ...
        internal static string[] GetQTSchoolProjects(string startPath)
        {
            return QueryDirectoryStructure(startPath, n => n.StartsWith("QT"), "bin", "obj", "node_modules");
        }
        internal static string[] GetQTSchoolPaths(string startPath, params string[] includePaths)
        {
            var qtProjects = GetQTSchoolProjects(startPath).Union(includePaths).ToArray();
        ...
        internal static string[] GetQTSchoolParentPaths(string startPath, params string[] includePaths)
        {
            var result = new List<string>();
            var qtProjects = GetQTSchoolProjects(startPath).Union(includePaths).ToArray();
            var qtPaths = qtProjects.Select(p => Program.GetParentDirectory(p))
